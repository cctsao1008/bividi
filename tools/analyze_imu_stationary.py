#!/usr/bin/env python3
"""Analyze a stationary Bividi Nori IMU trace without assuming sensor full-scale.
...
def vector_stats(samples: list[tuple[float, float, float]]) -> dict[str, dict[str, Any]]:
    if not samples:
...
def load_raw(path: Path) -> tuple[list[int], list[tuple[float, float, float]], list[tuple[float, float, float]], int]:
    timestamps: list[int] = []
    ...
    with stream:
        ...
        for line, row in enumerate(reader, start=2):
            ...
            try:
                ...
                accel.append(tuple(float(row[f"accel_raw_{axis}"]) for axis in "xyz"))
                gyro.append(tuple(float(row[f"gyro_raw_{axis}"]) for axis in "xyz"))
...
def analyze(
    ...
    *,
    ...
    timing = audit_imu_timing.audit(path)
    result: dict[str, Any] = {
        ...
        "timing": {
            ...
            "duplicate_timestamp_intervals": timing["imu"]["duplicate_timestamp_intervals"],
            "backward_timestamp_intervals": timing["imu"]["backward_timestamp_intervals"],
    ...
    if accel_g_per_count is not None:
        ...
        conversion["accelerometer_m_s2"] = vector_stats(scale_vector(accel_raw, factor))
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    if report["scale_conversion"] is not None:
        lines += ["", "## Explicit SI conversion", "", f"Scale source: `{report['scale_conversion'].get('source')}`", ""]
        for sensor, key, unit in (
            ...
            for axis in "xyz":
                ...
                lines.append(f"| {axis} | {fmt(stats['mean'])} | {fmt(stats['stddev_population'])} |")
            lines.append("")
    lines += ["", report["interpretation_note"], "", report["stationary_assumption"]]
    return "\n".join(lines)
...
def write_fixture(path: Path) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as stream:
        ...
        for index in range(8):
            ...
            ee = es + 2_000
            writer.writerow([
                frame, 100 + frame, 0, es, ee, es, ee, index % 4, "true",
                t, t,
                0 + (index % 2), 1, 8192, 10 + (index % 2), -20, 5,
            ])
...
def self_test() -> int:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        try:
            analyze(path, accel_g_per_count=1.0 / 8192.0, gyro_dps_per_count=None, scale_source=None)
        except ValueError as exc:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Analyze a stationary Bividi Nori IMU raw-count trace")

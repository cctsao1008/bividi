#!/usr/bin/env python3
"""Audit Bividi Nori IMU/camera timing from a lossless IMU trace CSV.
...
def load_trace(path: Path) -> tuple[list[FrameTiming], list[tuple[int, int]], int]:
    ...
    samples: list[tuple[int, int]] = []  # (extended_us, raw_us), valid only
    invalid_samples = 0
    ...
    with stream:
        ...
        for line, row in enumerate(reader, start=2):
            ...
            if frame is None:
                ...
                expected = (frame.sequence, frame.es_raw_us, frame.ee_raw_us, frame.es_us, frame.ee_us)
                actual = (sequence, es_raw, ee_raw, es, ee)
                if expected != actual:
                    raise ValueError(f"line {line}: inconsistent frame timing repeated for frame {frame_index}")
...
def audit(path: Path, gap_threshold_us: float | None = None) -> dict[str, Any]:
    ...
    if len(imu_times) > 1 and imu_times[-1] > imu_times[0]:
        effective_rate_hz = (len(imu_times) - 1) * 1_000_000.0 / (imu_times[-1] - imu_times[0])
    ...
    return {
        ...
        "camera": {
            ...
            "exposure_start_raw_counter_wraps": count_raw_wraps([frame.es_raw_us for frame in frames]),
            "exposure_end_raw_counter_wraps": count_raw_wraps([frame.ee_raw_us for frame in frames]),
        },
...
def write_fixture(path: Path) -> None:
    ...
    for frame in range(3):
        ...
        for sample_index in range(4):
            ...
            rows.append([
                frame, 100 + frame, 0, es, ee, es, ee, sample_index, "true",
                imu & 0xFFFFFFFF, imu, 1, 2, 3, 4, 5, 6,
            ])
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Audit camera/IMU device-time relationships from a Nori IMU trace")
    parser.add_argument("trace", nargs="?", type=Path)
    parser.add_argument("--gap-threshold-us", type=nonnegative_float,

...
def gates_of(role: str, data: Mapping[str, Any]) -> Mapping[str, Any]:
    if role == "calibration":
        quality = data.get("quality")
        return quality.get("gates", {}) if isinstance(quality, Mapping) and isinstance(quality.get("gates"), Mapping) else {}
...
def _check_bound_path(owner: Path, entry: Mapping[str, Any], label: str, findings: list[str]):
    path_text = entry.get("path")
    expected_hash = entry.get("sha256")
    if not isinstance(path_text, str) or not path_text or not isinstance(expected_hash, str) or len(expected_hash) != 64:
...
def check_session_sources(session: Mapping[str, Any], session_path: Path, require_promotion_sources: bool) -> list[str]:
    findings: list[str] = []
        ...
        if not isinstance(artifacts, list) or not artifacts:
            ...
            for index, entry in enumerate(artifacts):
                ...
                if not isinstance(role, str) or not role or role in roles:
                    findings.append(f"session.source_trace.artifacts[{index}] has invalid/duplicate role")
                    continue
                roles.add(role)
                _check_bound_path(session_path, entry, f"session.source_trace.{role}", findings)
    ...
    for index, pair in enumerate(pairs):
        ...
        for camera in ("camera_a", "camera_b"):
            ...
            if require_promotion_sources and (not isinstance(hash_text, str) or len(hash_text) != 64):
                findings.append(f"promotion requires immutable {camera} hash for pair {pair.get('pair_id', index)}")
                continue
...
def cross_check(data: Mapping[str, dict[str, Any]], paths: Mapping[str, Path], promotion=False) -> list[str]:
    findings = []
    target, scale, session, quality, calib, geometry, repeat = [
        data[key] for key in (
            "target", "target_scale", "session", "dataset_quality", "calibration", "geometry_review", "repeatability"
        )
    ]
...
def evaluate(manifest: dict[str, Any], owner: Path, docs_override=None, paths_override=None) -> dict[str, Any]:
    ...
    if profile == "promotion":
        ...
        if placeholder(mapping):
            findings.append("promotion requires #35 camera mapping evidence in session.capture.camera_mapping_evidence")
...
def self_test():
    with tempfile.TemporaryDirectory() as td:
        ...
        session = {
            ...
            "capture": {
                "mode_index": 0, "pixel_format": "GRAY8", "width": 640, "height": 480,
            ...
            "source_trace": {
                "kind": "test-recorder",
                "artifacts": [
                    {"role": "capture_manifest", "path": "capture.json", "sha256": sha(capture_path)},
                    {"role": "frames_csv", "path": "frames.csv", "sha256": sha(frames_path)},
                ],
            },
            "pairs": [{
                "pair_id": "p0", "camera_a": "a.png", "camera_b": "b.png",
                "camera_a_sha256": sha(camera_a_path), "camera_b_sha256": sha(camera_b_path),
            }],
        }
        session_path = root / "session.json"
        session_path.write_text(json.dumps(session), encoding="utf-8")
        session_hash = sha(session_path)
        scale = {"schema": TARGET_SCALE_SCHEMA, "target": {"sha256": target_hash}, "status": "PASS", "gates": {"g": {"pass": True}}, "policy_source": "lab-v1"}
        quality = {"schema": QUALITY_SCHEMA, "session": {"sha256": session_hash}, "target": {"sha256": target_hash}, "status": "PASS", "gates": {"g": {"pass": True}}, "policy_source": "lab-v1"}
        calib = {
            "schema": CALIB_SCHEMA, "calibration_id": "c",
            "provenance": {"kind": "measured", "source_session_sha256": session_hash},
            "device": session["device"], "capture": session["capture"], "target": {"sha256": target_hash},
            "quality": {"status": "PASS", "gates": {"g": {"pass": True}}, "policy_source": "lab-v1"},
        }
        calib_path = root / "calib.json"
        calib_path.write_text(json.dumps(calib), encoding="utf-8")
        calib_hash = sha(calib_path)
        geometry = {"schema": GEOMETRY_SCHEMA, "calibration": {"sha256": calib_hash}, "status": "PASS", "gates": {"g": {"pass": True}}, "policy_source": "lab-v1"}
        repeat = {"schema": REPEAT_SCHEMA, "artifacts": [{"sha256": calib_hash}], "status": "PASS", "gates": {"g": {"pass": True}}, "policy_source": "lab-v1"}
        files = {"target": target_path, "session": session_path, "calibration": calib_path}
        for name, data in (("target_scale", scale), ("dataset_quality", quality), ("geometry_review", geometry), ("repeatability", repeat)):
            path = root / (name + ".json")
            path.write_text(json.dumps(data), encoding="utf-8")
            files[name] = path
        namespace = argparse.Namespace(output=output, profile="promotion", policy_source="lab-v1", **files)
...
def parse(argv: Sequence[str] | None = None):
    ...
    for role in ROLES:
        build_parser.add_argument("--" + role.replace("_", "-"), dest=role, type=Path, required=True)
    build_parser.add_argument("--profile", choices=("integrity", "review", "promotion"), required=True)
